...
function Card({ icon, label, value, detail }: { icon: IconName; label: string; value: ReactNode; detail?: string }): ReactNode {
  return <article className="us-card"><div className="us-card-label"><Icon name={icon} size={16} />{label}</div><div className="us-card-value">{value}</div><div className={detail === undefined ? 'us-card-detail is-empty' : 'us-card-detail'} title={detail}>{detail ?? '\u00A0'}</div></article>
}
